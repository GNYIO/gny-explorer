<template>
  <b-card :title="formatTitle" class="shadow mt-4">
    <el-table class="clickable-rows" :data="dats" stripe v-loading="loading">
...
export default {
  computed: {
    ...mapGetters(['width']),
    formatTitle() {
      return 'My Registered DATs ' + '(total: ' + this.datsCount + ')';
    },
  },
  data() {
    return {
  ...
  watch: {
    ownerAddress: async function (ownerAddress) {
      console.log(`(Dats) ownerAddress changed to "${this.ownerAddress}"`);
      ...
      if (this.ownerAddress === null || this.ownerAddress === undefined || this.ownerAddress === '') {
        console.log(`(Dats) ownerAddress is "undefined"`);
        return;
      }
  ...
  methods: {
    handleCurrentChange: async function (currentPage) {
      this.loading = true;
      console.log(`(Dats) load data for page "${currentPage}"`);

      const from = (currentPage - 1) * this.pageSize;

      console.log(`offset: ${from}, limit: ${this.pageSize}, address: ${this.address}`);

      const raw = await connection.api.Dat.getDats({
        offset: from,
        limit: this.pageSize,
        ownerAddress: this.address,
      });
      this.dats = raw.dats;
      this.datsCount = raw.count;

      console.log(`(Dats) dats: ${JSON.stringify(this.dats, null, 2)}`);
